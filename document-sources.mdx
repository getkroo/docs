...
description: "Index SharePoint folders as document sources so Kris can search them"
icon: "folder-open"
---

Document sources let you index cloud storage folders — such as shared SharePoint sites — so their contents become searchable in Kris. This guide walks through creating a document template from a SharePoint site and copying it into your own sources.
...
- **SharePoint access** — the site and document library you want to index must be connected to Kroo
- **Admin permissions** in the Kroo App to create templates and manage document sources
...
<Step title="Fill in the template details">
  In the **New Document Template** dialog, provide the following:

  - **Display Name** — a name for the template, e.g. `Kroo SharePoint Test`
  - **Provider** — select **SharePoint**
  - **SharePoint Site** — start typing the site name and select it from the results (e.g. **Kroo Enterprise Search Testing**)
  - **Document Library** — choose a library from the site
  - **Folder Path** — *(optional)* a subfolder relative to the drive root, e.g. `/Contracts/Active`. Leave blank to index the whole drive.
